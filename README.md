...
Install **Base Fedora** using the [**Fedora Everything**](https://fedoraproject.org/misc/#everything) ISO. Just select locale, create user and set password, select Drive to install system on, enable & set root password (Optional), now hit next and watch it do its thing. No need to select any extra pacakages at this point, no need. Once it's done reboot and you will be on the TTY, login and run the following command :
