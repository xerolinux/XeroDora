...
Install **Base Fedora** using the [**Fedora Everything**](https://fedoraproject.org/misc/#everything) ISO. Just select locale, create user and set password, select Drive to install system on, enable & set root password (Optional), now hit next and watch it do its thing. No need to select any extra pacakages at this point, not required. Once it's done reboot and you will be on the TTY, login and run the following command :
...
> Use wired Internet as Fedora doesn't offer many WiFI drivers out the box post-base install. Not a Xero issue but a Fedora one. If you can't use Ethernet, try USB-Tethering. With that in mind install should go smoothly.
